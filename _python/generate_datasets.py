...
def generate_scarsity(length, num_series, scarsity_percentage):
    ...
    for _ in range(num_series):
        ...
        for _ in range(length):
            current_value = random.uniform(0, 20000)
...
def generate_delta(length, num_series, delta_percentage):
    ...
    for _ in range(num_series):
        ...
        time_series = []
        previous_value = random.uniform(0, 20000)  # Initialize the first value randomly
        for _ in range(length):
...
def generate_repeats(length, num_series, repeats_percentage):
    ...
    for _ in range(num_series):
        ...
        previous_value = random.uniform(0, 20000)  # Initialize the first value randomly
        for _ in range(length):
            possible_current_value = random.uniform(0, 20000)
...
def export(ts, file):
    try:
        os.mkdir("/mnt/c/Users/Albana Jaha/Desktop/MT/datasets/custom_data/")
    except FileExistsError:
        pass
    df = pd.DataFrame(ts).T
    df['time'] = [datetime.fromtimestamp((ms + i * 1000 * granularity) // 1000).strftime("%Y-%m-%dT%H:%M:%S") for i in
                  range(len(df))]  # 2019-03-01T00:00:00
    ...
    df.to_csv('/mnt/c/Users/Albana Jaha/Desktop/MT/datasets/custom_data/' + file, index=False)
    return df


length = 10000
num_series = 100
outlier_percentages = [0, 1, 2, 5, 10, 20]
repeats_percentages = [0, 10, 30, 50, 70, 90]
scarsity_percentages = [0, 10, 30, 50, 70, 90]
delta_percentages = [0, 1, 2, 5, 100, 2000, 10000]
